OUTPUT_DIR="sd-rgbd-model-lora-3"
ACC_CONFIG="gpu3"
...
accelerate launch --config_file $CONFIG train_text_to_image_lora.py \
  ...
  --train_batch_size=4 \
  --num_train_epochs=2 --checkpointing_steps=10000 \
  --conv_learning_rate=1e-03 --learning_rate=1e-03 --lr_scheduler="constant" --lr_warmup_steps=0 \
  --seed=42 \
