...
DATA_DIR="/data/NYU_processed/NYU_processed.py"
DATA_SPLIT="train"
HF_DATASETS_OFFLINE=1
TRANSFORMERS_OFFLINE=1

accelerate launch --config_file $CONFIG --num_processes=1 train_text_to_image_lora.py \
  --pretrained_model_name_or_path=$MODEL_NAME\
  --dataset_name=$DATA_DIR --dataset_split=$DATA_SPLIT --caption_column="text" \
  --resolution=512 --random_flip --lora_rank=4\
  --train_batch_size=2 \
  --num_train_epochs=1 --checkpointing_steps=10000 \
